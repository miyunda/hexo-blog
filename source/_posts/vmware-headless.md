...
### 缘起
之前的两年我一直使用Parallels Desktop（下称PD），无奈这公司吃相太难看，变着法er地割韭菜；而Virtual Box（下称VBox）总赶脚在稳定性和性能上不太舒服，~~其实是UI太丑~~；VMware就成了虚拟机御三家中最后的选择——其他产品太小众就不考虑了。
迁移到VMware还是比较简单的，VMware提供了向导，直接导入PD虚拟机即可。
然后用着用着我就想VMware有木有VBox那样的无头模式，不启动图形界面，多少能省点资源。
#### vmrun
...
然后给我的虚拟机`.vmx`文件建个环境变量，也是为了方便使用，我使用的是ZSH，用BASH的同学自行替换下：
```bash
